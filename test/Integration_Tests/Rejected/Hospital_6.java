...
class Hospital extends Object {
    ...
    function end_trial (id : int {|R1|rho|} => {||rho|}) -> bool {|p0|} {
        id.revoke(R1);                                                                  
        return true;
    }
}
...
class Main extends Object {
    ...
    function Main(){
        ...
        let bob_id : int {|R1, PressRelease|} := 123 in {
            hospital1.enroll_patient(bob_id);  // TODO() Fix this 
            hospital1.end_trial(bob_id);
            // bob_id.grant(R1)           // This is how it is fix
            hospital1.enroll_patient(bob_id); // rejects!
                                              // Bob no longer contains
                                              // R1. bob needs to grant it back
        }
    }
}